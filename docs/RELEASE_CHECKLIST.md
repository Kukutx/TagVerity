...
- [ ] If saved privacy settings are unreadable, history remains hidden and scanning is blocked until an explicit recovery applies the complete current privacy policy; interim toggle changes do not load/rewrite history, and the confirmed delete-only recovery path clears persisted history without loading it while leaving scanning paused.
## Android
