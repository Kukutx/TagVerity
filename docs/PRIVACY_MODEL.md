...
## Storage integrity
History and settings are stored locally through `shared_preferences` for the current small dataset. Persisted JSON is shape-validated on load and before save, including schema-critical fingerprint format, byte-formatted raw UIDs, unique technologies, unique scan IDs, and unique NDEF record indexes. Invalid outgoing history is rejected before it can replace the last known-good stored copy.
