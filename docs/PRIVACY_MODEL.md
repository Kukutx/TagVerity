...
## Storage integrity
History and settings are stored locally through `shared_preferences` for the current small dataset. Persisted history JSON is shape-validated on load and before save, including schema-critical fingerprint format, byte-formatted raw UIDs, unique technologies/scan IDs, sequential NDEF indexes, strict NDEF identifier/preview hex, NDEF preview-length and payload/record-length consistency, known scan/NDEF top-level fields, and a 500-record compatibility ceiling matching the maximum supported by early releases. Invalid outgoing history is rejected before it can replace the last known-good stored copy. Early v2 history that predates `identityStability` remains readable; its original fingerprint construction is used to recover stable versus session-only identity before the controller reapplies current privacy settings.
