...
## Storage integrity
History and settings are stored locally through `shared_preferences` for the current small dataset. Persisted history JSON is shape-validated on load and before save, including schema-critical fingerprint format, byte-formatted raw UIDs, unique technologies, unique scan IDs, unique NDEF record indexes, and known scan/NDEF top-level fields. Invalid outgoing history is rejected before it can replace the last known-good stored copy. Early v2 history that predates `identityStability` remains readable and is normalized by the controller.
