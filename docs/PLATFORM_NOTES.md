...
- The fixed scan timeout remains active after a tag is discovered until inspection finishes. If a platform metadata/NDEF operation stalls, the session is invalidated and later completion is ignored; native session-close waits are themselves bounded to 5 seconds.
