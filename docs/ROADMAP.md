...
- [x] Keep NDEF display summaries within the declared 300-character total bound, including the truncation ellipsis.
...
- [x] Persisted history is raw-shape/type validated on load and before save, then checked through the shared collection/model scan contract for unique event IDs, fingerprint/UID/identity, technology, and NDEF invariants; the legacy 500-record ceiling and early-v2 identity recovery remain compatible.
- [x] Scan models are immutable snapshots: nested scan collections are defensively copied/frozen and JSON DTO collections do not alias back into validated scan state.
...
- [x] Privacy-safe diagnostics with bounded/redacted recursively immutable event snapshots, JSON-safe non-finite numeric normalization, schema-bounded nested data, and explicit privacy-recovery export state.
### UX and quality
...
- [x] Android debug plus unsigned ARM32+ARM64 release AAB/ABI verification, final merged-manifest NFC/no-INTERNET checks, iOS NFC metadata/entitlement/Xcode-wiring validation, unsigned iOS release compile, and immutable commit-SHA pinning for remote GitHub Actions with Dependabot maintenance.
