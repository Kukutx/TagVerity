...
- Promoted CI build coverage to release paths: every PR now builds an unsigned ARM32+ARM64 Android release AAB, verifies the expected Flutter runtime ABIs/no x86 runtime, and compiles iOS in release mode without signing; the project validator protects these release gates from silent removal.
