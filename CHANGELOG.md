...
- The fixed scan timeout now remains active through tag inspection instead of stopping at discovery, so stalled metadata/NDEF reads cannot leave scanning active indefinitely; duplicate discoveries are ignored while one inspection owns the session, and native session-close waits are bounded to 5 seconds.
