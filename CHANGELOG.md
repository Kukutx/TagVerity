...
- The fixed scan timeout now remains active through tag inspection instead of stopping at discovery, so stalled metadata/NDEF reads cannot leave scanning active indefinitely; duplicate discoveries are ignored while one inspection owns the session. Native session-close waits are bounded to 5 seconds, but an unconfirmed close is not treated as success: scan delivery/rearm is suppressed and replacement native sessions remain blocked until the outstanding close actually settles.
