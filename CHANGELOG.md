...
- Persistence plus JSON/CSV export now share one scan contract, including collection-level unique event IDs. Raw UID retention must match its SHA-256 fingerprint and comparable identity, and export preparation failures surface cleanly instead of producing schema-invalid reports or uncaught UI errors.
- `NfcScan` now snapshots and exposes immutable technology/detail/NDEF/warning collections, while `toJson()` emits independent collection copies. This prevents a validated scan from being mutated later through constructor aliases, getters, copy inputs, or serialized DTO aliases. The unused partial `includeRawUid` serializer flag was removed so privacy minimization stays in the explicit history policy instead of a one-field JSON shortcut.
...
- If saved privacy settings cannot be decoded, TagVerity now fails closed: saved history stays hidden and untouched, scanning/history mutations pause, privacy toggles can be composed without reading or rewriting history, and only an explicit apply action loads history under the complete current policy. Recovery is serialized with normal settings writes.
- Privacy recovery now also offers a confirmed delete-only path that clears all saved history without loading it first; deletion failures leave the hidden persisted copy intact, and successful deletion still keeps scanning paused until the current privacy policy is applied.
...
- Sanitized diagnostic nested maps/lists are now recursively immutable snapshots, preventing retained/redacted event data from being changed later through consumer references before export. Diagnostics schema v4 also normalizes non-finite numbers, formally models bounded nested values, and exports explicit privacy-settings recovery state so hidden history counts cannot be mistaken for an empty persisted store.
...
- Promoted CI build coverage to release paths: every PR now builds an unsigned ARM32+ARM64 Android release AAB, verifies the expected Flutter runtime ABIs/no x86 runtime, inspects the final merged manifest for required NFC/no INTERNET permission, and compiles iOS in release mode without signing; the project validator also enforces Android NFC/FileProvider plus iOS usage/AID/TAG entitlement and Debug/Profile/Release Xcode wiring so these release gates cannot silently drift. Remote GitHub Actions are pinned to immutable commit SHAs while Dependabot remains enabled for `github-actions` updates.
